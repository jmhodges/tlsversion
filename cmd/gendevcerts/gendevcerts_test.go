...
// `curl --cacert ./config/development_ca_cert.pem`: a generated leaf that
// shares an identical subject DN with the CA and carries no Authority Key Id
// looks self-signed to OpenSSL/curl (verify error 18), so they never chain it
// to the CA. Go's own verifier is lenient about this case and happily verifies
// the handshake, so the TLS tests in the main package can't catch it — we
// assert the OpenSSL-relevant invariants here instead.
